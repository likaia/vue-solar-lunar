 ...
 * @param year int 年份
 */
function _checkYear(year: number) {
  if (year < minYear) {
 ...
 * @param month int 月份
 */
function _checkMonth(month: number) {
  if (month < 1) {
 ...
 */
export function getLunarYearMonthDaysCount(year: number, month: number) {
 ...
 * @param day int 日期
 */
function getLunarDayString(day: number) {
  day = Math.floor(day);
  if (_checkDay(day)) {
    const nStr1 = [
      "日",
 ...
 * @param n 节气编号 0~23
 */
export function getLunarTermDay(year: number, n: number) {
 ...
 * @param month int 月份 1~12
 */
export function getSolarMonthDaysCount(year: number, month: number) {
  if (_checkYear(year) && _checkMonth(month)) {
    if (month === 2) {
 ...
 * @param day int 日期
 */
export function getSolarDayNumber(year: number, month: number, day: number) {
...
export function solarToLunar(year: number, month: number, day: number) {
  if (_checkYear(year) && _checkMonth(month) && _checkDay(day)) {
    const baseDate: any = new Date(minYear, 0, 31); // 基础日期1900年1月31日
    const objDate: any = new Date(year, month - 1, day); // 目标日期
    let offset = (objDate - baseDate) / 86400000; // 偏移天数 60 * 60 * 24 * 1000 = 86400000，1天的毫秒数
    let temp = 0;
    let i;

    for (i = minYear; i < maxYear && offset > 0; i++) {
      temp = getLunarYearDaysCount(i) as number; // 农历year年的总天数
      if (offset < temp) {
        break;
      } else {
        offset -= temp;
    ...
    const leap = getLunarLeapMonth(lunarYear) as number; // 当年闰月是哪个月
    ...
    for (i = 1; i <= 12 && offset > 0; i++) {
      if (leap > 0 && i === leap + 1 && !isLeapMonth) {
        ...
        isLeapMonth = false;
        temp = getLunarYearMonthDaysCount(lunarYear, i) as number;
      }
    ...
    return {
      year: lunarYear, // 农历年份
      ncWeek: weekday[objDate.getDay()], // 星期几
      month: lunarMonth, // 农历月份
      day: Math.floor(lunarDay), // 农历日期
      isLeap: isLeapMonth, // 是否闰月
      Terms: getLunar24Days(year, month, day - 1), // 节气
