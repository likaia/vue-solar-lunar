 ...
 * @param year int 年份
 */
export function getLeapMonthDaysCount(year) {
 ...
 * @param year int 年份
 */
export function getLunarYearDaysCount(year) {
    if (_checkYear(year)) {
 ...
 */
export function getLunarYearMonthDaysCount(year, month) {
 ...
 * @param n 节气编号 0~23
 */
export function getLunarTermDay(year, n) {
    if (_checkYear(year) && n <= 23 && n >= 0) {
        ...
        const offDate = new Date(31556925974.7 * (year - minYear) +
            sTermInfo[n] * 60000 +
 ...
 * @param month int 月份 1~12
 */
export function getSolarMonthDaysCount(year, month) {
    if (_checkYear(year) && _checkMonth(month)) {
        if (month === 2) {
 ...
 * @param day int 日期
 */
export function getSolarDayNumber(year, month, day) {
...
export function solarToLunar(year, month, day) {
    if (_checkYear(year) && _checkMonth(month) && _checkDay(day)) {
        ...
        return {
            year: lunarYear,
            ncWeek: weekday[objDate.getDay()],
            month: lunarMonth,
            day: Math.floor(lunarDay),
            isLeap: isLeapMonth,
            Terms: getLunar24Days(year, month, day - 1),
            monthStr: monthStr,
